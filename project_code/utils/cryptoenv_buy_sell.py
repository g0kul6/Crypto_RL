...
class Environment(gym.Env):
    ...
    def reset(self):
        ...
        if self.random:
            self.t = np.random.randint(self.ws,len(self.data)-500)
        else :
        ...
        # self.agent_positions = [self.data.iloc[self.t-1] for _ in range(240)]
        self.buy_value = self.data.iloc[self.t-1]["Close"]
        self.positions = []
        self.steps = 0
        self.no_buys = 0
        self.no_sells = 0
    ...
    def step(self, act):
        ...
        reward = 0
        # EXECUTE THE ACTION (act = 0: buy, 1: sell)
        if act == 0:  # Do Nothing
        ...
        if act == 1:  # Sell
            ...
            profit = self.data.iloc[self.t]["Close"] - self.buy_value 
        ...
        if self.env_type == "train":
            if act == 0:
                # if sell at time self.t
                if_sell_profit = self.data.iloc[self.t]["Close"] - self.buy_value
                if if_sell_profit<0:
                    reward = 1
